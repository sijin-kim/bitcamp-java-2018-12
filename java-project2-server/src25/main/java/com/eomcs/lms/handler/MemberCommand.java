...
public class MemberCommand {
  ...
  public void list(ServletRequest request,ServletResponse response) throws Exception {
    ...
    out.println("<p><a href='/member/form'>회원목록 새 글</a></p>");
    out.println("<table border='1'>");
    out.println("<tr> <th>번호</th> <th>이름</th> <th>이메일</th> <th>전화번호</th> <th>가입날</th></tr>");
    
    
    
    for (Member member : members) {
      out.println(String.format(
          "<tr><td>%d</td> <td><a href='/board/detail?no=%1$d'>%s</a></td> <td>%s</td> <td>%s</td><td>%s</td></tr>",
          member.getNo(),
          member.getName(), 
          member.getEmail(),
          member.getTel(),
