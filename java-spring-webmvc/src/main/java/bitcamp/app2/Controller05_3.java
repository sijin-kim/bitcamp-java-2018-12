...
public class Controller05_3 {
  ...
  public String handler3(@RequestBody String content) throws Exception {
  ...
  public Object handler5(@RequestBody Board content) throws Exception {
    ...
    //주의!
    // => 클라이언트에서 보낸 날짜 데이터의 문자열 형식이 yyy-mm-dd 형태여야 한다.
    // 그래야 java.util.Date객체 변환 성공~
    // => 만약 이 형태가 아니면 변환할수 없어 실행 오류가 발생한다.
    // 예)2019-5-1 ===> 변환 오류~
    
    //@jsonformat 애테이션 사용
    //=>이 애노테이션은 MappingJackson2HttpMEssageConverter가 인식하는 애노테이션이다.
    // => 도메인 객체의 프로퍼티에 이 애노테이션을 붙이면
    // 2019-05-01 이나 2019-5-1 모두 처리할수 있다.
    //=> 뿐만 아니라, 도메인 객체를 json 문자열로 변환할 떄도
    // 해당 형식으로 변환된다.
